...
class TypeSystem(ts.Origami):
    ...
    def Source(self, env, expr, ty):
        ...
        env['Vec@1@List[Int]->Vec'] = ts.Def(None, None, '{x: ${1}, y: ${2}}')
        VecType = expr.ofType('Vec')
        for v in expr[1:]:
            print('@@@', v)
    ...
    def Name(self, env, expr, ty):
        ...
        return findname(env, name, expr)

    def GetExpr(self, env, expr, ty):
        name = fieldName(env, expr[2])
        ebase = self.asType(env, expr[1], None)
        print('@GetExpr', name, expr)
        if ebase == '#JSON':
            c = ebase[1]
            if name in c:
                return c[name]
    ...
    def Adj(self, env, expr, ty):
        return parseAdjective(env, expr, ty)

    def Verb(self, env, expr, ty):
        return parseVerb(env, expr, ty)
...
def perrorObject(env, ename, name=None):
    if name is None: name = str(ename)
    perror(env, ename.getpos(), '(型エラー) オブジェクトではありません'.format(name))

def perrorObjectName(env, ename, name=None):
    if name is None: name = str(ename)
    perror(env, ename.getpos(), '(名前エラー) 「{}」は定義された属性名ではありません'.format(name))
...
def findname(env, name, expr):
    if name in RAW:
        return expr.new2('#Raw', RAW[name])
    if name.endswith('色'):
        return findname(env, name[:-1], expr)
    print('@@findname', name, fieldName(env, getThat(env)))
...
def staticEval(env, e):
    name = str(ename)
    if name in env.ext.proto:
        c = env.ext.proto[name]
        return e.new2('#JSON', c)
    if e == '#GetExpr':
        c = staticEval(e[1])
        name = fieldName(e[2])
        if c.tag == '#JSON':
            ...
            perrorObjectName(env, e[2])
        else:
            perrorObject(env, e[1])
    return e.new2('#Null', None)
...
def setStaticField(env, c, ename, evalue):
    ...
    setThat(env, ename)
    evalue = env.ts.asType(env, evalue, type)
    print('@', name, evalue)
    if evalue == '#Of':
        name = str(evalue[1])
        evalue = evalue[2]
...
#def assume(env, ename, everb, eblock = None):
#    name = str(ename)
#    if name in WORLD:


def staticAssign(env, ename, evalue):
    '''
    if ename == '#GetExpr':
        expr.tag = '#SetExpr'
        expr[1] = self.asType(env, ename, None)
    ...
    if name in WORLD:
        name = WORLD[name]
        c = env.ext.proto['世界']
    elif name in env.ext.proto:
        c = env.ext.proto[name]
        name = 'some'
    if c is not None:
        setStaticField(env, c, name, evalue)
    else:
        perrorUndefinedName(env, ename)
...
def parseAdjective(env, eadj, ty):
    v = find(str(eadj), ADJ)
    if v is not None:
        return eadj.new('#Of', v[1], eadj.new2('#Raw', v[2]))
    return eadj.new2('#Null', v)
